...
URL = "https://www.iaa.gov.il/airports/ben-gurion/flight-board/?flightType=arrivals"
DB_PATH = Path("data/flights.db")
RAW_JSON_PATH = Path("debug/ben_gurion_arrivals_raw.json")


def ensure_table(conn: sqlite3.Connection) -> None:
    conn.execute("""
    CREATE TABLE IF NOT EXISTS flights_ben_gurion_raw (
...
def save_rows(rows: List[Dict]) -> None:
    ...
    conn.execute("DELETE FROM flights_ben_gurion_raw")

    for row in rows:
        conn.execute("""
        INSERT INTO flights_ben_gurion_raw (
...
def scrape_rows() -> List[Dict]:
    with sync_playwright() as p:
        ...
        for i in range(count):
            ...
            if len(cells) >= 8:
                # הסדר האמיתי באתר:
                # [airline, flight_number, origin_city, terminal, scheduled_time, scheduled_date, updated_time, status]
                rows.append({
                    "airline": cells[0],
